function [Xk, Ck] = InEKF_UpdateLeft(kdMap, ptMap, ptHdl, Cbk, XbkTF, ParaS)
...
if size(vMapPts, 1) > 20   % the prediction value of observed points
    ...
    for i = 1 : 1 : size(vZbk,1)
        Hx = -[(skew(tmpp') - skew(vMapPts(i,:))) * tmpR, tmpR]; % left        
        Sk = Hx * Cbk * Hx' + Qk;
        Kk = Cbk * Hx' / Sk;
        eState = eState * Exp(Kk * (vZbk(i,:)' - vMapPts(i,:)')); % Left
        Cbk = (eye(6) - Kk * Hx) * Cbk;
    end
    %-----------------------------------------------------
    Xk =  XbkTF * eState; % left-invariant
    Ck = Cbk;
    %-----------------------------------------------------
else
    Xk = XbkTF;
